...
fn proxy_defaults(interface: &str) -> Option<(&'static str, Option<&'static str>)> {
    match interface {
        ...
        "org.gnome.Mutter.ScreenCast" => Some((
        ...
        "org.gnome.Shell.Introspect" => Some((
...
fn apply_proxy_defaults(interface: &str, iface_impl: String) -> String {
    ...
    let new_attr = match path {
        Some(path) => format!(
            "#[proxy(interface = \"{interface}\", default_service = \"{service}\", default_path = \"{path}\")]"
        ),
    ...
    proxy_attr_re
        .replacen(&iface_impl, 1, new_attr)
        .into_owned()
}
